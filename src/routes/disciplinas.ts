...
router.post("/", async (req, res) => {
  ...
  const result = await db.run(
    "INSERT INTO disciplinas (nome, codigo, instituicao_id, periodo, criado_em, atualizado_em) VALUES (?, ?, ?, ?, ?, ?)",
    nome,
    codigo || null,
    instituicao_id,
    periodo || null,
...
router.put("/:id", async (req, res) => {
  ...
  if (formula != null) {
    const componentes = await db.all(
      "SELECT id, sigla FROM componentes_nota WHERE disciplina_id = ? ORDER BY id",
      id
    );
  ...
  await db.run(
    "UPDATE disciplinas SET nome = ?, codigo = ?, instituicao_id = ?, formula = ?, atualizado_em = ? WHERE id = ?",
    nome || existing.nome,
...
router.delete("/:id", async (req, res) => {
  ...
  const turmas = await db.get(
    "SELECT COUNT(1) as cnt FROM turmas WHERE disciplina_id = ?",
    id
  );
