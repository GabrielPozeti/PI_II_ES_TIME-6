/*
  Arquivo: src/routes/auth.ts
  Finalidade: Rotas de autenticação e gerenciamento de conta (registro, login,
  recuperação de senha, logout e informações do usuário).
  Observações: Usa utilitários de `db` e `utils/email` e opera sobre tokens de reset.
*/

import express from "express";
import bcrypt from "bcryptjs";
import crypto from "crypto";
import { db } from "../db";
import fs from "fs";
import path from "path";
import { sendEmail } from "../utils/email";

const router = express.Router();

router.post("/register", async (req, res) => {
  const { nome, email, telefone, senha } = req.body;
  if (!nome || !email || !senha)
    return res.status(400).json({ message: "Campos incompletos" });

  const database = await db();
  const existing = await database.get(
    "SELECT * FROM docentes WHERE email = $1",
    [email]
  );
  if (existing)
    return res.status(409).json({ message: "E-mail já cadastrado" });

  const senha_hash = bcrypt.hashSync(senha, 10);
  const result = await database.run(
    "INSERT INTO docentes (nome,email,telefone,senha_hash) VALUES ($1,$2,$3,$4) RETURNING *",
    [nome, email, telefone || null, senha_hash]
  );

  return res.status(201).json({
    message: "Registrado",
    user: result.lastID ? await database.get("SELECT * FROM docentes WHERE id = $1", [result.lastID]) : null
  });
});

router.post("/login", async (req, res) => {
  const { email, senha } = req.body;
  if (!email || !senha)
    return res.status(400).json({ message: "Campos incompletos" });

  const database = await db();
  const row = await database.get(
    "SELECT * FROM docentes WHERE email = $1",
    [email]
  );
  if (!row) return res.status(401).json({ message: "Credenciais inválidas" });

  const match = bcrypt.compareSync(senha, row.senha_hash);
  if (!match) return res.status(401).json({ message: "Credenciais inválidas" });

  // Cria sessão
  (req as any).session.userId = row.id;

  return res.json({
    message: "Logado",
    user: {
      id: row.id,
      nome: row.nome,
      email: row.email,
      telefone: row.telefone,
      curso: row.curso,
      id_instituicao: row.id_instituicao,
    },
  });
});

router.post("/forgot-password", async (req, res) => {
  const { email } = req.body;
  if (!email) return res.status(400).json({ message: "Email é obrigatório" });

  const database = await db();
  const row = await database.get("SELECT * FROM docentes WHERE email = $1", [email]);

  if (!row) {
    return res.json({ message: "Se o e-mail existir, você receberá instruções" });
  }

  const token = crypto.randomBytes(24).toString("hex");
  const expires = Date.now() + 15 * 60 * 1000; // 15 minutos
  const dataDir = path.resolve(__dirname, "..", "..", "data");
  const tokensFile = path.join(dataDir, "reset_tokens.json");

  let tokens: Record<string, { userId: number; expires: number }> = {};
  if (fs.existsSync(tokensFile)) {
    try { tokens = JSON.parse(fs.readFileSync(tokensFile, "utf-8") || "{}"); } catch { tokens = {}; }
  }

  tokens[token] = { userId: row.id, expires };
  if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
  fs.writeFileSync(tokensFile, JSON.stringify(tokens, null, 2));

  const resetLink = `http://localhost:3000/reset-password.html?token=${token}`;
  await sendEmail(email, row.nome, resetLink);

  return res.json({ message: "Se o e-mail existir, você receberá instruções" });
});

router.post("/reset-password", async (req, res) => {
  const { token, senha } = req.body;
  if (!token || !senha)
    return res.status(400).json({ message: "Token e nova senha são obrigatórios" });

  try {
    const dataDir = path.resolve(__dirname, "..", "..", "data");
    const tokensFile = path.join(dataDir, "reset_tokens.json");
    if (!fs.existsSync(tokensFile))
      return res.status(400).json({ message: "Token inválido ou expirado" });

    let tokens: Record<string, { userId: number; expires: number }> = {};
    try { tokens = JSON.parse(fs.readFileSync(tokensFile, "utf-8") || "{}"); } catch { tokens = {}; }

    const record = tokens[token];
    if (!record || record.expires < Date.now())
      return res.status(400).json({ message: "Token inválido ou expirado" });

    const database = await db();
    const user = await database.get("SELECT * FROM docentes WHERE id = $1", [record.userId]);
    if (!user) return res.status(400).json({ message: "Usuário inválido" });

    const senha_hash = bcrypt.hashSync(senha, 10);
    await database.run("UPDATE docentes SET senha_hash = $1 WHERE id = $2", [senha_hash, record.userId]);

    delete tokens[token];
    fs.writeFileSync(tokensFile, JSON.stringify(tokens, null, 2));

    return res.json({ message: "Senha atualizada" });
  } catch {
    return res.status(400).json({ message: "Erro ao processar solicitação" });
  }
});

router.post("/logout", (req, res) => {
  const s = (req as any).session;
  if (s) s.destroy?.(() => {});
  return res.json({ message: "Desconectado" });
});

router.post("/me", async (req, res) => {
  const { userId } = req.body;
  if (!userId) return res.status(400).json({ message: "ID do usuário é obrigatório" });

  const database = await db();
  const user = await database.get("SELECT * FROM docentes WHERE id = $1", [userId]);
  if (!user) return res.status(404).json({ message: "Usuário não encontrado" });

  return res.json({ user });
});

router.put("/info-adicional", async (req, res) => {
  const { instituicao, curso, id } = req.body;
  if (instituicao === undefined || curso === undefined || id === undefined)
    return res.status(400).json({ message: "Campos incompletos" });

  try {
    const database = await db();
    let instituicaoId: number | null = null;

    let instituicaoDB = await database.get("SELECT * FROM instituicoes WHERE nome = $1", [instituicao]);
    if (!instituicaoDB) {
      const sigla = instituicao.slice(0, 3).toUpperCase();
      const resInsert = await database.run("INSERT INTO instituicoes (nome, sigla) VALUES ($1,$2) RETURNING id", [instituicao, sigla]);
      instituicaoId = resInsert.lastID || null;
    } else {
      instituicaoId = instituicaoDB.id;
    }

    await database.run("UPDATE docentes SET id_instituicao = $1, curso = $2 WHERE id = $3", [instituicaoId, curso, id]);

    const updatedUser = await database.get("SELECT * FROM docentes WHERE id = $1", [id]);
    if (!updatedUser) return res.status(404).json({ message: "Usuário não encontrado" });

    return res.json({ message: "Informações adicionais atualizadas", user: updatedUser });
  } catch (error) {
    return res.status(500).json({ message: "Erro ao atualizar informações" });
  }
});

export default router;
