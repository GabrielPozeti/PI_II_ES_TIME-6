...
router.get("/componente/:id", async (req, res) => {
  ...
  const notasRows = await database.all(
    "SELECT aluno_id, valor FROM notas WHERE componente_id = ?",
    [componenteId]
  );
...
router.put("/", async (req, res) => {
  ...
  if (!componente_id || !Array.isArray(notas)) {
    return res
      .status(400)
      .json({ message: "componente_id e notas são obrigatórios" });
  }
  ...
  try {
    ...
    for (const item of notas) {
      ...
      // Se remover nota → sem log
      if (raw == null || raw === "") {
        await database.run(
          "DELETE FROM notas WHERE aluno_id = ? AND componente_id = ?",
      ...
      if (Number.isNaN(v) || v < 0 || v > 10) {
        ...
        return res
          .status(400)
          .json({ message: `Valor inválido para aluno ${aluno_id}: ${raw}` });
      }

      const aluno = await database.get(
        "SELECT a.id, a.nome, t.disciplina_id FROM alunos a JOIN turmas t ON a.id_turma = t.id WHERE a.id = ?",
        [aluno_id]
      );
      if (!aluno) {
        await database.run("ROLLBACK");
      ...
      if ((aluno as any).disciplina_id !== (comp as any).disciplina_id) {
        await database.run("ROLLBACK");
        return res.status(400).json({
          message: `Aluno ${aluno_id} não pertence à disciplina do componente`,
        });
      }

      // Buscar valor anterior
      const notaAnterior = await database.get(
        "SELECT valor FROM notas WHERE aluno_id = ? AND componente_id = ?",
        [aluno_id, componente_id]
      );
      ...
      if (!notaAnterior) {
        mensagem = `${formatDateTime()} - (Aluno ${
          aluno.nome
        }) - Nota ${v} lançada pela primeira vez.`;
      } else if (notaAnterior.valor !== v) {
        mensagem = `${formatDateTime()} - (Aluno ${aluno.nome}) - Nota de ${
          notaAnterior.valor
        } para ${v} modificada e salva.`;
      }

      // Salvar nota
      await database.run(
        `INSERT INTO notas (aluno_id, componente_id, valor, atualizado_em)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT(aluno_id, componente_id)
         DO UPDATE SET valor = EXCLUDED.valor, atualizado_em = EXCLUDED.atualizado_em`,
        [aluno_id, componente_id, v, now]
      );

      // Inserir log (se necessário)
      if (mensagem) {
        await database.run(
          `INSERT INTO auditoria_notas(aluno_id, componente_id, valor_antigo, valor_novo, mensagem)
           VALUES (?, ?, ?, ?, ?)`,
          [aluno_id, componente_id, notaAnterior?.valor ?? null, v, mensagem]
        );
      }
