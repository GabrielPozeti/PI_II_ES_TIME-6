...
router.post("/", async (req, res) => {
    ...
    const result = await db.run("INSERT INTO disciplinas (nome, codigo, instituicao_id, periodo, criado_em, atualizado_em) VALUES (?, ?, ?, ?, ?, ?)", nome, codigo || null, instituicao_id, periodo || null, now, now);
    ...
    if (formula != null) {
        ...
        if (missingSigla.length > 0)
            return res.status(400).json({
                message: "Todos os componentes precisam ter sigla antes de salvar a fórmula",
            });
        ...
        if (notUsed.length > 0)
            return res.status(400).json({
                message: "A fórmula deve utilizar todas as siglas dos componentes cadastrados",
                missing: notUsed.map((c) => c.sigla),
            });
    }
