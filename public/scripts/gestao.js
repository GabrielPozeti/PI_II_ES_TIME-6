...
async function loadInstituicoes() {
  ...
  for (const i of data) {
    ...
    edit.onclick = () => {
      document.getElementById("instituicao-id").value = i.id;
      document.getElementById("instituicao-nome").value = i.nome;
      document.getElementById("instituicao-sigla").value = i.sigla || "";
    };
    const del = document.createElement("button");
    del.textContent = "Excluir";
    del.onclick = () => {
      pendingDelete = { type: "instituicao", id: i.id };
      modalText.textContent = `Confirma exclusão da instituição ${i.nome}?`;
      modal.className = "";
    };
...
async function loadDisciplinas() {
  ...
  for (const d of data) {
    const li = document.createElement("li");
    li.textContent = `${d.nome} ${d.codigo ? "- " + d.codigo : ""} (${
      d.instituicao_nome
    })`;
    const actions = document.createElement("div");
    actions.className = "item-actions";
    ...
    edit.onclick = () => {
      ...
      document.getElementById("disciplina-instituicao").value =
        d.instituicao_id;
    };
    const del = document.createElement("button");
    del.textContent = "Excluir";
    del.onclick = () => {
      pendingDelete = { type: "disciplina", id: d.id };
      modalText.textContent = `Confirma exclusão da disciplina ${d.nome}?`;
      modal.className = "";
    };
    actions.appendChild(edit);
    actions.appendChild(del);
    li.appendChild(actions);
    listaDisc.appendChild(li);
    const opt = document.createElement("option");
    opt.value = d.id;
...
async function loadTurmas() {
  ...
  for (const t of data) {
    ...
    edit.onclick = () => {
      ...
      document.getElementById("turma-codigo").value = t.codigo || "";
      document.getElementById("turma-periodo").value = t.periodo || "";
      document.getElementById("turma-disciplina").value = t.disciplina_id;
    };
    const del = document.createElement("button");
    del.textContent = "Excluir";
    del.onclick = () => {
      pendingDelete = { type: "turma", id: t.id };
  ...
  try {
    ...
    for (const a of data) {
      ...
      edit.onclick = () => {
        ...
        if (document.getElementById("aluno-turma"))
          document.getElementById("aluno-turma").value = a.id_turma;
      };
      const del = document.createElement("button");
      del.textContent = "Excluir";
      del.onclick = () => {
        pendingDelete = { type: "aluno", id: a.id };
        modalText.textContent = `Confirma exclusão do aluno ${a.nome}?`;
        modal.className = "";
      };
      actions.appendChild(edit);
      actions.appendChild(del);
...
formInst.addEventListener("submit", async (e) => {
  e.preventDefault();
  const id = document.getElementById("instituicao-id").value;
  const nome = document.getElementById("instituicao-nome").value;
  const sigla = document.getElementById("instituicao-sigla").value;
  try {
    if (id) await request("PUT", `/instituicoes/${id}`, { nome, sigla });
    else await request("POST", "/instituicoes", { nome, sigla });
...
document.getElementById("instituicao-cancel").addEventListener("click", () => {
  document.getElementById("instituicao-id").value = "";
  document.getElementById("instituicao-nome").value = "";
  document.getElementById("instituicao-sigla").value = "";
});
formDisc.addEventListener("submit", async (e) => {
  ...
  const instituicao_id = document.getElementById(
    "disciplina-instituicao"
  ).value;
  try {
    if (id)
      await request("PUT", `/disciplinas/${id}`, {
        nome,
        codigo,
        instituicao_id,
      });
    else
      await request("POST", "/disciplinas", { nome, codigo, instituicao_id });
...
document.getElementById("disciplina-cancel").addEventListener("click", () => {
  document.getElementById("disciplina-id").value = "";
  document.getElementById("disciplina-nome").value = "";
  document.getElementById("disciplina-codigo").value = "";
});
formTur.addEventListener("submit", async (e) => {
  e.preventDefault();
  ...
  const disciplina_id = document.getElementById("turma-disciplina").value;
  try {
    if (id)
      await request("PUT", `/turmas/${id}`, { codigo, periodo, disciplina_id });
    else await request("POST", "/turmas", { codigo, periodo, disciplina_id });
...
document.getElementById("turma-cancel").addEventListener("click", () => {
  document.getElementById("turma-id").value = "";
  document.getElementById("turma-codigo").value = "";
  document.getElementById("turma-periodo").value = "";
});
...
modalConfirm.addEventListener("click", async () => {
  ...
  try {
    if (pendingDelete.type === "instituicao")
      await request("DELETE", `/instituicoes/${pendingDelete.id}`);
    else if (pendingDelete.type === "disciplina")
      await request("DELETE", `/disciplinas/${pendingDelete.id}`);
    else if (pendingDelete.type === "turma")
      await request("DELETE", `/turmas/${pendingDelete.id}`);
    else if (pendingDelete.type === "aluno")
      await request("DELETE", `/alunos/${pendingDelete.id}`);
    pendingDelete = null;
    modal.className = "modal-hidden";
    await loadInstituicoes();
    await loadAlunos();
...
formAluno &&
  formAluno.addEventListener("submit", async (e) => {
    ...
    try {
      if (id)
        await request("PUT", `/alunos/${id}`, { matricula, nome, id_turma });
...
document.getElementById("aluno-cancel") &&
  document.getElementById("aluno-cancel").addEventListener("click", () => {
    document.getElementById("aluno-id").value = "";
    document.getElementById("aluno-matricula").value = "";
    document.getElementById("aluno-nome").value = "";
  });
